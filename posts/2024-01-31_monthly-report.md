...
6章 高度な型と7章 TypeScriptのモジュールシステム終わった。  
8章 非同期処理をやってる。そろそろ終わりそうだけど期間が空き過ぎて正直あんまり意味なかった。
...
丁度ブルーベリー本の写経が非同期の章なので、直列で商品検索してる処理を非同期で並列実行できないかと模索してみたけど、最初のログイン処理を手動でやってる関係でセッションをWebDriver間で共有する方法がわからず撃沈。  
...
### [僕のヒーローアカデミア #1-5](https://amzn.to/4bcjt9o)
